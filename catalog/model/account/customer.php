...
class ModelAccountCustomer extends Model {
	...
	public function addLoginAttempt($phone) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_login WHERE phone = '" . $this->db->escape(utf8_strtolower((string)$phone)) . "' AND ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "'");

		if (!$query->num_rows) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer_login SET phone = '" . $this->db->escape(utf8_strtolower((string)$phone)) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', total = 1, date_added = '" . $this->db->escape(date('Y-m-d H:i:s')) . "', date_modified = '" . $this->db->escape(date('Y-m-d H:i:s')) . "'");
		} else {
			$this->db->query("UPDATE " . DB_PREFIX . "customer_login SET total = (total + 1), date_modified = '" . $this->db->escape(date('Y-m-d H:i:s')) . "' WHERE customer_login_id = '" . (int)$query->row['customer_login_id'] . "'");
		}
	}

	public function getLoginAttempts($phone) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer_login` WHERE phone = '" . $this->db->escape(utf8_strtolower($phone)) . "'");
		return $query->row;
	}

	public function deleteLoginAttempts($phone) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "customer_login` WHERE phone = '" . $this->db->escape(utf8_strtolower($phone)) . "'");
	}
}
